        ...
        <form id="userForm" class="border rounded">
            ...
            <div class="mb-3">
                ...
                <select class=" form-select" id="userBooks">
                    ${
                        this.library.getItems().filter((book) => book.available)
                            .length === 0
    ...
    private attachEventListenersToBooksForm() {
        ...
        if (bookForm) {
            bookForm.addEventListener("submit", (e) => {
                ...
                const titleWarning = document.getElementById(
                    "titleWarning",
                ) as HTMLSpanElement;
                const authorWarning = document.getElementById(
                    "authorWarning",
                ) as HTMLSpanElement;
                const yearWarning = document.getElementById(
                    "yearWarning",
                ) as HTMLSpanElement;
                ...
                if (year < 0 || year > 2024) {
                    ...
                    yearWarning.textContent =
                        "Рік повинен бути в межах 0 - 2024";
                    yearWarning.style.display = "inline";
                }
    ...
    private attachEventListenersToUsersForm() {
        ...
        if (userForm) {
            userForm.addEventListener("submit", (e) => {
                ...
                const emailFormatWarning = document.getElementById(
                    "emailFormatWarning",
                ) as HTMLSpanElement;
        ...
        if (addBookForUserButton) {
            addBookForUserButton.addEventListener("click", (e) => {
                ...
                if (userBooks.querySelectorAll(`option`).length === 1) {
                    userBooks.innerHTML +=
                        '<option disabled="disabled" selected>Книжок не знайдено</option>';
                }
            });
        }
    }
